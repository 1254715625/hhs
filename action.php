...
if($payment){
	$this_attr=$db->getRow("select * from mcs_task_taobao a,mcs_member_bindacc b where a.id=$payment and a.get_user=b.id and b.uid=$uinfo[user_id]  and a.process=0");
	if($this_attr){
		...
		if($attr['isViewEnd']==1){
			...
			if ($picname != "") {
				...
				$db->execute("update mcs_task_taobao set finish='".$finish."' where id=$payment");
...
if($pid){
	$this_attr=$db->getRow("select * from mcs_task_taobao a,mcs_member_bindacc b where a.id=$pid and a.get_user=b.id and b.uid=$uinfo[user_id] and a.process=2");
	if($this_attr['id']){
		...
		if($attrs['pinimage']==1){
			...
			if ($picname != "") {
				...
				$finish=serialize($finish);
				$db->execute("update mcs_task_taobao set finish='".$finish."' where id=$pid");
				$size = round($picsize/1024,2);
				$arr = array('name'=>$picname,'pic'=>$pics,'size'=>$size);
				echo json_encode($arr);
			}
