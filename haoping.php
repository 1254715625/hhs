...
if($pid){
	$this_attr=$db->getField("mcs_task_taobao a,mcs_member_bindacc b","attrs","a.id=$pid and a.get_user=b.id and b.uid=$uinfo[user_id] and a.process=2");
	if($this_attr){
		$this_attr=unserialize($this_attr);
		if($this_attr['pinimage']==1){
?>
<meta charset="utf-8">
<style type="text/css">
...
$(function () {
	...
	var files = $(".files");
	var btn = $(".btn span");
	$(".demo").wrap("<form id='myupload' action='action.php?pid=<?php echo $pid;?>' method='post' enctype='multipart/form-data'></form>");
