...
/*
	...
	$cfg['port']    = '25';
	$cfg['user']    = '253356420@qq.com';
	$cfg['pass']    = '**********';
	$cfg['mail']    = '253356420@qq.com';
	$cfg['charset'] = 'gbk';
*/
function send_mail($name, $email, $subject, $content, $type = 0, $notification=false)
{
	$site_name = 'haohuishua';
    $charset   = $GLOBALS['cfg']['charset'];
	
    /**
     * 使用mail函数发送邮件
     */
    if ($GLOBALS['cfg']['service'] == 0 && function_exists('mail'))
    {
        /* 邮件的头部信息 */
        $content_type = ($type == 0) ? 'Content-Type: text/plain; charset=' . $charset : 'Content-Type: text/html; charset=' . $charset;
        $headers = array();
        $headers[] = 'From: "' . '=?' . $charset . '?B?' . base64_encode($site_name) . '?='.'" <' . $GLOBALS['cfg']['smtp_mail'] . '>';
        $headers[] = $content_type . '; format=flowed';
        if ($notification)
        {
            $headers[] = 'Disposition-Notification-To: ' . '=?' . $charset . '?B?' . base64_encode($site_name) . '?='.'" <' . $GLOBALS['cfg']['smtp_mail'] . '>';
        }
    ...
    else
    {
        ...
        $headers[] = 'From: "' . '=?' . $charset . '?B?' . base64_encode($site_name) . '?='.'" <' . $GLOBALS['cfg']['account'] . '>';
        ...
        if ($notification)
        {
            $headers[] = 'Disposition-Notification-To: ' . '=?' . $charset . '?B?' . base64_encode($site_name) . '?='.'" <' . $GLOBALS['cfg']['account'] . '>';
        }

        /* 获得邮件服务器的参数设置 */
        $params['host'] = $GLOBALS['cfg']['smtp'];
        $params['port'] = $GLOBALS['cfg']['port'];
        $params['user'] = $GLOBALS['cfg']['account'];
        $params['pass'] = $GLOBALS['cfg']['password'];
        ...
        else
        {
			...
			include_once './source/class/smtp.class.php';
            static $smtp;

            $send_params['recipients'] = $email;
            $send_params['headers']    = $headers;
            $send_params['from']       = $GLOBALS['cfg']['account'];
