...
if(IS_POST&&IS_AJAX&&$_POST['safepass']=='safepass'){
	if($uinfo['safepass'] == cipherStr($_POST['pass'])){
		setcookie('safepass',cipherStr($uinfo['user_id']),time()+3600*12,'/','.haohuisua.com');
		$state['state']=true;
		$state['str']='安全码验证成功！';
	}else{
		setcookie('safepass','',time()-1);
...
if($temid){
	$templates=$db->getRow("select value from mcs_task_template where uid=$uinfo[user_id] and id=$temid and type='$mod' and tem_type='pp'");
...
$umaihaoh=$db->getAll("select id from mcs_member_bindacc where acc_type='pp' and uid={$uinfo['user_id']} and buyno=0 and is_hang=1 and hang_time<".strtotime(date("Y-m-d")));
foreach($umaihaoh as $val){
	$db->execute("update mcs_member_bindacc set is_hang=0,hang_time=0,unhang_time=".time()." where id=".$val['id']);
}

$umaihaol=$db->getAll("select id from mcs_member_bindacc where acc_type='pp' and uid={$uinfo['user_id']} and buyno=0 and is_lock=1 and lock_time<".(time()-30*24*3600));
foreach($umaihaol as $val){
	$db->execute("update mcs_member_bindacc set is_lock=0,lock_time=0,unlock_time=".time()." where id=".$val['id']);
}

$maihao=$db->getAll("select id from mcs_member_bindacc where acc_type='pp' and uid={$uinfo['user_id']} and buyno=0 and is_hang=0 and is_lock=0");
foreach($maihao as $val){
	$user_state=$db->getCount('mcs_task_paipai','get_user='.$val['id'].' and get_time>'.strtotime(date("Y-m-d")));
