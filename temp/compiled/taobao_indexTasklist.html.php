<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tlisttable">
          ...
          <thead>
	        <tr>
	            ...
	            <td width="21%" align="center" valign="middle" class="tico rwdt_bt2"><span class="h-sort" val="0" style="width:80px;margin-left: 30px;" onclick="getTask();">任务编号</span></td>
	            <td width="12%" align="left" valign="middle" class="tico rwdt_bt2">发布人</td>
	            <td width="12%" align="left" valign="middle" class="tico rwdt_bt2"><span class="h-sort" val="1" onclick="getTasks('gettask','goods_price')">任务价格</span></td>
	            <td width="23%" align="center" valign="middle" class="tico rwdt_bt2"><span class="h-sort" val="2" style="width:93px" onclick="getTasks('gettask','ddlOKDay')">发布者要求</span></td>
	            <td width="22%" align="left" valign="middle" class="tico rwdt_bt2"><span class="h-sort" val="3" onclick="getTasks('gettask','txtMinMPrice')">悬赏刷点</span></td>
...
<tr class="rwdt_list <?php if($this->_var['key'] % 2 != 0){ ?>bgcollor<?php } ?>"><td width="10">&nbsp;</td>
<td width="21%">
<span class="hico type<?php echo $this->_var['vals']['task_types']; ?> topt" title="来路任务">
		<strong>TB<?php echo $this->_var['vals']['id']; ?></strong>
	</span>
	<br>
    <?php if($this->_var['vals']['cbxIsSB'] == 1){ ?><span class="hico limit1 lopt" title="接任务者需要加入平台商保">Time:&nbsp;<?php echo $this->_var['vals']['addtime']; ?><?php if($this->_var['vals'] [ attrs ] [ isMobile ] == 1){ ?><img src="themes/default/images/public/sjdd.png" style="vertical-align: sub;margin-left:5px;" title="接任务者需要通过手机、pad等智能设备的app下单支付"><?php } ?></span><a href="user.php?act=ensure" title="接任务者的平台商保金额需要大于150.00元，可额外获得0.3个刷点" style="width: 150px;float: left;text-align: center;">接任务需要加入平台商保</a><?php }else{ ?>Post Time:&nbsp;<?php echo $this->_var['vals']['addtime']; ?><?php if($this->_var['vals'] [ attrs ] [ isMobile ] == 1){ ?><img src="themes/default/images/public/sjdd.png" style="vertical-align: sub;margin-left:5px;" title="接任务者需要通过手机、pad等智能设备的app下单支付"><?php } ?>
	<?php } ?>
    </span>
</td>
<td width="12%">
    <a title="接任务后方可查看到对方QQ号码" target="_blank" href="info.php?act=info&uname=<?php echo $this->_var['vals']['add_user']; ?>" class="<?php if($this->_var['vals']['special'] == 1){ ?>orange strong<?php }else{ ?>blue<?php } ?>">
...
<td width="23%" class="noktime">
	...
	<?php if($this->_var['vals']['needimg'] == 1){ ?><span style="color: #fff; background:#4CB5EE;border-radius:10%; display:inline-block;width: 60px;">好评晒图</span><?php } ?> <?php if($this->_var['vals']['cbxIsLook'] == 1){ ?><span style="color: #fff; background:#4CB5EE;border-radius:10%; display:inline-block;width: 60px;">浏览店铺</span><?php } ?>
</td>
