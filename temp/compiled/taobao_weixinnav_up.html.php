...
<div class="tico menu">
	<a href="?mod=showweixin" <?php if($this->_var['mod'] == 'showweixin'){ ?>class="nov"<?php } ?> title="微信任务大厅">微信任务大厅</a>
	<a href="?mod=weixininTask" <?php if($this->_var['mod'] == 'weixininTask'){ ?>class="nov"<?php } ?> title="已接任务">已接任务</a>
	<a href="?mod=weixinoutTask" <?php if($this->_var['mod'] == 'weixinoutTask'){ ?>class="nov"<?php } ?> title="已发任务">已发任务</a>
	<a href="?mod=bindweixin" <?php if($this->_var['mod'] == 'bindweixin'){ ?>class="nov"<?php } ?> title="绑定买号">绑定微信号</a>
	<span class="rwdt_lx" style="right: -215px;position: relative;bottom:-5px;">
  任务编号：<input type="text" style="height: 22px;border: 1px solid #B2CAFF;" id="taskinfo"><button style="border: 1px solid #B2CAFF;background: white;margin-left: 5px;color: #777;height: 24px;" id="taskid" onclick="getTasksearch('gettask');">搜索</button>
	...
	<!-- <a href="?mod=bindseller" <?php if($this->_var['mod'] == 'bindseller'){ ?>class="nov"<?php } ?> title="绑定掌柜">绑定掌柜</a> -->
	<!-- <a href="sou.php" title="搜索任务大厅" style="color: white; background: url(themes/default/images/taobao/t.gif) -133px -1px no-repeat;">搜索任务大厅</a> -->
</div>
