$(function(){
	$("#nAuTotk").click(function(){
		var self=this;
		window.ints=0;
		if(!$(this).attr('checked'))return ;
		$.post("paipai.php?mod=autoam",function(data){
			if(data.status == 1){
				art.dialog({ 
					title: '温馨提示',
					content: data.info,
					...
					cancelValue: '取消', 
					ok: function () {
						window.location.href='home.php?act=buypoint';
					},
					cancel: function(){
						$("#nAuTotk").attr('checked',false);
					}
				});
			}else{
				art.dialog({ 
	...
	$('.fb_btn').click(function(){	
		location.href = "paipai.php?mod=addTask";
	});
	...
	$('#addSeller').click(function(){
		...
		if(nickname == '')
		{
			alert('请输入您要绑定的掌柜名称');
			return false;
		}else if(nickname.indexOf('输入QQ名称')>=0){
			$('#nickName').val('');
			$('#nickName').focus();
			return false;
		}else{
			var win = art.dialog({content : '正在查询，请稍候...',lock : true});
	...
	$('.bindBuyer').click(function(){
		...
		var win = art.dialog({title:'提示',content : '正在查询，请稍候...',lock : true});
