...
{	
	...
	$("#tloading").show();
	$.get("sou.php?mod=gettask&search="+a, obj, function(data, status){
		if(data){
			$("#tloading").hide();
		}
		$("#taskList").html(data);
...
{	
	if(type){
		...
		if(search){
			$("#tloading").show();
			$.get("sou.php?mod="+type+"&search="+search,  function(data, status){
				if(data){
					$("#tloading").hide();
				}
				$("#taskList").html(data);
...
function getinTask(get)//已接刷新
{
	...
	$("#tloading").show();
	$.get("sou.php?mod=getinTask", get, function(data, status){
...
function getTasks(mod,obj){
	...
	$.get("sou.php?mod="+mod+"&order="+obj+"&desc="+desc, function(data, status){
		$("#taskList").html(data);
	});
}
function getTasksa(mod,obj){
	...
	$.get("sou.php?mod="+mod+"&order="+obj+"&desc="+desc+"&process="+process, function(data, status){
		$("#taskList").html(data);
	});
}
...
{	
	...
	$("#tloading").show();
	$.get("sou.php?mod=getoutTask", get, function(data, status){
		if(data){
			$("#tloading").hide();
		}
		$("#taskList").html(data);
...
function qiang(id){//抢任务
	...
	if(id){
		$.get("sou.php?mod=qiang&id="+id, function(data, status){
			if(data=='no'){
				art.dialog({title : '请绑定买号',
				  content : '抱歉，您没有可以使用的买号',
				  okValue : "绑定买号",
				  ok : function(){
							location.href="sou.php?mod=bindBuyer"
				  },
				cancelValue : "取消",
			...
			var win = art.dialog({title : '请选择买号',
				  ...
				  ok : function(){
					if(data!='不允许接自己发布的任务'){
							$.get("sou.php?mod=qiang&id="+id+"&gid="+$("input[name='buyer']:checked").val(), function(data, status){
								if(data){
								art.dialog({
								id:'id-info',
								content:data,
							...
							var timer = setInterval(function () {
							    ...
							    if (i === 0) {
									...
									dialog.ok(location.href="sou.php?mod=inTask");
							    };
							}, 1000);
							});}
						},
...
function payment(id){//已付款
	if(id){
			$.get("sou.php?mod=getinTask&payment="+id,function(data,status){
				...
				art.dialog({title : '温馨提示',
				  ...
				  ok : function(){
						if(data=='need'){
							...
							if(has==1){
								$.get("sou.php?mod=getinTask&payment="+id+"&f=1", function(data, status){
									getinTask();
								});
							}else{
								art.dialog({title : '温馨提示',
...
function nopayment(id){//取消已付款
	if(id){
			art.dialog({title : '温馨提示',
				  ...
				  okValue : "确定",
				  ok : function(){
					$.get("sou.php?mod=getinTask&nopayment="+id, function(data, status){
								getinTask();
					});
				},
				cancelValue : "取消",
...
function task_out(obj,id){//任务退出
	if(id){
			art.dialog({title : '温馨提示',
				  ...
				  okValue : "确定",
				  ok : function(){
					$.get("sou.php?mod=task_out&id="+id, function(data, status){
						getinTask();
					});
				},
				cancelValue : "取消",
...
function getTaobaourl(id){//查看商品地址
	if(id){
			$.get("sou.php?mod=getTaobaourl&id="+id, function(data, status){
				art.dialog({title : '温馨提示',
				  content : data,
				  okValue : "打开地址",
				  ok : function(){
...
function praise(id){//好评
	if(id){
			art.dialog({title : '温馨提示',
				  ...
				  okValue : "确定",
				  ok : function(){
				$.get("sou.php?mod=getinTask&praise="+id, function(data, status){
							location.reload();
				});
				},
				cancelValue : "取消",
...
function stop(id){//暂停
	if(id){
			art.dialog({title : '温馨提示',
				  ...
				  okValue : "确定",
				  ok : function(){
						$.get("sou.php?mod=getoutTask&stop="+id, function(data, status){
...
function yifa(id,un){//已发货
	if(id){
			art.dialog({title : '温馨提示',
				  ...
				  okValue : "确定",
				  ok : function(){
						$.get("sou.php?mod=getoutTask&yifa="+id, function(data, status){
								art.dialog({title : '温馨提示',
								  content : "恭喜，您已发货成功！",
								  okValue : "确定",
								  ok : function(){
...
function queren(id){//确认收货
	if(id){
			art.dialog({title: false,
				  ...
				  okValue : "确定",
				  ok : function(){
						$.get("sou.php?mod=getoutTask&queren="+id, function(data, status){
								getoutTask();
						});
				},
				cancelValue : "取消",
...
function huifu(id){//恢复
	if(id){
			art.dialog({title : '温馨提示',
				  ...
				  okValue : "确定",
				  ok : function(){
						$.get("sou.php?mod=getoutTask&huifu="+id, function(data, status){
	...
	if(id){
		art.dialog({title : '备注（最多255个字）',
				  ...
				  ok : function(){
						...
						$.get("sou.php?mod=remarks&id="+id+"&remarks="+remarks, function(data, status){
								art.dialog({title : '备注（最多255个字）',
								  content : "备注成功",
								  okValue : "确定",
								  ok : function(){
...
function shenshu(id){//申诉
	if(id){
		var win=art.dialog({title : '任务申诉',
				  content : "<iframe src='sou.php?mod=appeal&id="+id+"' width='550px' height='370px' frameborder='0' scrolling='no' name='con' id='con'></iframe>",
				  okValue : "确定",
				  ok : function(){
						...
						if(info==''){
							...
							if(content==0){
								...
								$.post('sou.php?mod=appeal',{"id":id,"info":info,"content":content},function(data){
									if(data.state){
			...
			art.dialog({title : '温馨提示',
			  ...
			  ok : function(){
					...
					$.post('?mod=pingjia&id='+id,{"grade":grade,"remark":remark,"isHide":isHide},function(data){
						alert(data);
						if(data=='ok'){
							art.dialog({title : '温馨提示',content : "恭喜，评论成功，谢谢您对好会刷平台所做的贡献",okValue : "确定",ok : function(){return true;},lock : true});	
						}
						return true;
					});
				...
				ok:function(){	
					...
					if(ue){
						...
						if(!expcontarr || expcontarr.length != lxnum){
							...
							$.getJSON('sou.php?mod=jt',{'shopBrGoods':expcont,'id':id},function(data){
								alert(data);
								if(data.error){
					...
					if (goodsurl==''){
						art.dialog({id:'temp-infos',title : '温馨提示',content :'商品URL不能为空',okValue : "确定",ok : function(){return true;},lock : true});
						return false;
					}
					...
					if(suess=='商品地址验证成功！'&&img==1){
						...
						art.dialog({id:'temp-infos',title : '温馨提示',content :'请验证商品地址',okValue : "确定",ok : function(){return true;},lock : true});
						return false;
					}
