$(function(){
	$("#nAuTotk").click(function(){
		...
		$.post("taobao.php?mod=autoam",function(data){
			if(data.status == 1){
				art.dialog({ 
					title: '温馨提示',
					content: data.info,
				...
				var ints=window.setInterval(function(){
						auto(data.attr.taobao);
				},3000);
			}
	   },'json');
	});
	var hdfot_top = jQuery.cookie('hdfot_top');
	if(hdfot_top == 1){
		$(".hdfot").hide();
		$(".hdfots").show();
		$(".bw").hide();
	...
	$('#addSeller').click(function(){
		...
		if(nickname == '')
		{
			...
			var win = art.dialog({content : '正在查询掌柜信息，请稍候...',lock : true});
	...
	$('.bindBuyer').click(function(){
		...
		var win = art.dialog({content : '正在查询买号信息，请稍候...',lock : true});
