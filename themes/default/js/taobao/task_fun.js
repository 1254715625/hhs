function auto(attr){
	if(window.ints==1){
		var limits = [],limitsTwe = [];
		var obj = attr;
		$.post('taobao.php?mod=gettask_list',function(info){
			for(var i=0,l=info.length;i<l;i++){
				for(var key in info[i]){
					if(info[i]['goods_price'] <= parseInt(obj.txtMinPrice) || info[i]['goods_price'] >= parseInt(obj.txtMaxPrice)) return false;
					if(info[i]['total_points'] <= parseInt(obj.txtMinpoint)) return false;
					if(info[i]['cbxIsSB'] && obj.NoSB) return false;
					if(info[i]['attrs']['cbxIsAudit'] && obj.NoAudit) return false;
					if(info[i]['attrs']['cbxIsWW'] && obj.NoWW) return false;
					if(info[i]['attrs']['cbxIsLHS'] && obj.NoLHS) return false;
					if(info[i]['isSign'] && obj.Notask1) return false;
					if(info[i]['attrs']['isViewEnd'] && obj.Notask2) return false;
					if(info[i]['attrs']['pinimage'] && obj.Notask3) return false;
					if(info[i]['attrs']['isViewEnd'] && obj.Notask4) return false;
					if(info[i]['attrs']['isCompare'] && obj.Notask5) return false;
					if(info[i]['attrs']['stopDsTime'] && obj.Notask6) return false;
					if(info[i]['attrs']['cbxIsWX'] && obj.Notask7) return false;
					if(info[i]['ddlOKDay'] && obj.NoYanchi) return false;
					if(obj.typeorder == 1){
						if(info[i]['attrs']['isMobile']) return false;
					}
					if(obj.typeorder == 2){
						if(!info[i]['attrs']['isMobile']) return false;
					}
					if(obj.buyerDeng == 1){
						if(info[i]['attrs']['isBuyerFen'] || info[i]['attrs']['ismgo']){
							return false;
						}
					}
					if(info[i]['task_type'] == 4 && obj.NoCart) return false;
					if(info[i]['task_type'] == 3 && obj.NoMeal) return false;
					if(info[i]['task_type'] == 2 && obj.NoLaiLu) return false;
					if(info[i]['isSign'] && obj.NoExpress) return false;
					if(info[i]['attrs']['pinimage'] && obj.NoPinPic) return false;
					if(parseInt(obj.timesBegin)-1 > info[i]['ddlOKDay']) return false;
					if(parseInt(obj.timesEnd)-1 < info[i]['ddlOKDay']) return false;
					if(obj.isCartOnly){if (info[i]['task_type'] == 4) return true; else return false;}
					if(obj.isMealOnly){if (info[i]['task_type'] == 3) return true; else return false;} 
					if(obj.isLailuOnly){if (info[i]['task_type'] == 2) return true; else return false;}
					if(obj.isExOnly){if (info[i]['isSign']) return true; else return false;} 
					if(obj.isEnsureOnly){if (info[i]['cbxIsSB']) return true; else return false;}
					qiang(info[i]['id']);
...
{	
	var t_type='';
	if(obj){t_type=obj.t_type;}
	$(".sx_btn").attr({'data':'getTask','alt':t_type});
	$("#tloading").show();
...
function getweixinTask(obj)//大厅刷新
{	
	var t_type='';
	if(obj){t_type=obj.t_type;}
	$(".sx_btn").attr({'data':'getTask','alt':t_type});
	$("#tloading").show();
	$.get("taobao.php?mod=getweixintask", obj, function(data, status){
		// console.log(data);
		if(data){
			$("#tloading").hide();
		}
...
function sx(){
	var data=$(".sx_btn").attr('data');
	var alt=$(".sx_btn").attr('alt');
	switch(data){
		case 'getTask':
			if(alt){
				getTask({t_type:alt});
...
{
	var process='';
	if(get){process=get.process;}
	$(".sx_btn").attr({'data':'getinTask','alt':process});
	$("#tloading").show();
...
function getweixininTask(get)//已接刷新
{
	var process='';
	if(get){process=get.process;}
	$(".sx_btn").attr({'data':'getinTask','alt':process});
	$("#tloading").show();
	$.get("taobao.php?mod=getweixininTask", get, function(data, status){
		// alert(data);
		if(data){
			$("#tloading").hide();
		}
...
function getweixinfinish(get)//已接完成
{
	var process='';
	if(get){process=get.process;}
	$(".sx_btn").attr({'data':'getinTask','alt':process});
	$("#tloading").show();
	$.get("taobao.php?mod=getweixinfinish", get, function(data, status){
		// alert(data);
		if(data){
			$("#tloading").hide();
		}
...
{	
	var process='';
	if(get){process=get.process;}
	$(".sx_btn").attr({'data':'getoutTask','alt':process});
	console.log(process)
	$("#tloading").show();
...
function getweixinoutTask(get)//已发刷新
{	
	var process='';
	if(get){process=get.process;}
	$(".sx_btn").attr({'data':'getoutTask','alt':process});
	console.log(process)
	$("#tloading").show();
	$.get("taobao.php?mod=getweixinoutTask", get, function(data, status){
		if(data){
			$("#tloading").hide();
		}
		$("#taskList").html(data);
...
function getweixinoutfinish(get)//已发刷新
{	
	var process='';
	if(get){process=get.process;}
	$(".sx_btn").attr({'data':'getoutTask','alt':process});
	console.log(process)
	$("#tloading").show();
	$.get("taobao.php?mod=getweixinoutfinish", get, function(data, status){
		if(data){
			$("#tloading").hide();
		}
		$("#taskList").html(data);
...
function tdopen(index){
	var path = 'themes/default/images/taobao/';
	$('.chooseBuyer ul').hide();
	$('.chooseBuyer ul.true-name-'+index).show();
	$('.tdOpenTrueNameBa').find('img').attr({src : path+'show.png'});
	$('.chooseBuyer .tdOpenTrueNameBa:eq('+index+')').find('img').attr({src : path+'hidden.gif'});
}
	...
	if(id){
		$.get("taobao.php?mod=qiang&id="+id, function(data, status){
			...
			if(data=='no'){
				...
				cancelValue : "取消",
				cancel : function(){window.ints=1;},
				 
				  lock : true});
			}else{
			var win = art.dialog({title : '请选择买号',
				  content : data,
				  okValue : "确定",
				  ok : function(){
					if(data!='不允许接自己发布的任务'){
							...
							var dialog = art.dialog.get('id-info');
							var i = 5;
...
function weixinqiang(id){//抢任务
	// alert(id);
	if(id){
		$.get("taobao.php?mod=weixinqiang&id="+id, function(data, status){
			// console.log(data);
			// console.log(status);
			if(data=='no'){
				art.dialog({title : '请绑定买号',
			...
			var win = art.dialog({title : '请选择买号',
				  ...
				  ok : function(){
					if(data!='不允许接自己发布的任务'){
							...
							$.get("taobao.php?mod=weixinqiang&id="+id+"&gid="+$("input[name='buyer']:checked").val(), function(data, status){
								if(data){
									// alert(data);
								art.dialog({
								id:'id-info',
							...
							var timer = setInterval(function () {
							    ...
							    if (i === 0) {
									...
									dialog.ok(location.href="taobao.php?mod=weixininTask");
							    };
							}, 1000);
							});}
						},
				  ...
				  ok : function(){
						if(data=='need'){
							...
							if(has==1){
								...
								art.dialog({title : '温馨提示',
								  content : '<form action="taobao.php?mod=underimg&id='+id+'" method="post" enctype="multipart/form-data"><input type="file" name="underimg"><input type="submit" value="上传"></form>',
								  okValue : "取消",
								  ok : function(){
										return true;
								  }, 
...
function weixinment(id){//已付款
	if(id){
			$.get("taobao.php?mod=getweixininTask&payment="+id,function(data,status){
				var con="<p style='color:red;'>【注意】该任务为微信投票任务</p><p>请确保已经投票后，才进行平台确认，否则一经投诉，将做封号处理！</p>";
				if(data=='need'){
					con+="<p style='margin-top: 10px;'><span style='color:#fe5500;'>此任务需要上传商品浏览底图</span></p><iframe src='upload.php?payment="+id+"' width='350' height='120' frameborder='0' name='frm'></iframe>";
				}
				art.dialog({title : '温馨提示',
				  ...
				  ok : function(){
						...
						// 	}else{
								art.dialog({title : '温馨提示',
								  content : '<form action="taobao.php?mod=weixinunderimg&id='+id+'" method="post" enctype="multipart/form-data"><input type="file" name="underimg"><input type="submit" value="上传"></form>',
								  // okValue : "确定",
								//   ok : function(){
								// 		return true;
								// }, 
						...
						// 	}
						// }else{
						// 	$.get("taobao.php?mod=getweixininTask&payment="+id+"&f=1", function(data, status){
						// 		alert(data);
						// 		if(data=='ok'){
									// getweixininTask();
						// 		}
			...
			art.dialog({title : '温馨提示',
				  ...
				  ok : function(){
						$.get("taobao.php?mod=getoutTask&stop="+id, function(data, status){
								var textadd = $(".yfrw_sub li:eq(1) .tips-number .whitedit2");
								var anum = parseInt(textadd.html()) + 1; 
								textadd.html(anum);
								var textkcl = $(".yfrw_sub li:eq(0) .tips-number .reddit2");
								var anums = parseInt(textkcl.html()) - 1; 
								if(anums<0){anums=0;}
								textkcl.html(anums);
								$(".yfrw_sub li:eq(1) a").addClass('addone').html("<b>+1</b>");
								sx();
						});
				},
				cancelValue : "取消",
...
function yifa(id,un){//已发货
	if(id){
			art.dialog({title : '温馨提示',
				  content : '<div style="width:420px"><p style="float: left;color: #000;font-size: 14px;">请确认买家账号：<a style=" font-size: 15px; font-weight: bold; color: #0085FF; " >'+un+'</a>  是否已经拍下付款，确认立即在网店发货吗？</p><p style="color: #888888; float: left; width: 100%;margin-top: 10px;">如果没有付款请勿发货，<span style=" color: #F33737;">要求您发货，索要账号密码、验证码的客服都属于假客服，切勿相信。</span>买家长时间不拍下付款请进行<a style="font-size: 13px;font-weight: bold;color: #3995E9;">投诉处理</a>。</p></div>',
				  okValue : "确定",
				  ok : function(){
...
function weixinqueren(id){//确认收货
	if(id){
			art.dialog({title: false,
				  ...
				  okValue : "确定",
				  ok : function(){
						$.get("taobao.php?mod=getweixinoutTask&queren="+id, function(data, status){
								getweixinoutTask();
						});
				},
				cancelValue : "取消",
			...
			art.dialog({title : '温馨提示',
				  ...
				  ok : function(){
						$.get("taobao.php?mod=getoutTask&huifu="+id, function(data, status){
								var textadd = $(".yfrw_sub li:eq(0) .tips-number .reddit2");
								var anum = parseInt(textadd.html()) + 1; 
								textadd.html(anum);
								var textkcl = $(".yfrw_sub li:eq(1) .tips-number .whitedit2");
								var anums = parseInt(textkcl.html()) - 1; 
								if(anums<0){anums=0;}
								textkcl.html(anums);
								$(".yfrw_sub li:eq(0) a").addClass('addone').html("<b style='margin:-6px 38px;'>+1</b>");
								sx();
						});
				},
				cancelValue : "取消",
	...
	if(id){
		var win=art.dialog({title : '任务申诉',
				  ...
				  ok : function(){
						...
						if(info==''){
							var x=art.dialog({title : '任务申诉',content : '请选择投诉类型',okValue : "确定",ok : function(){x.close();win.show();},lock : true});
							win.show();
						}
						else{
							var content=con.window.imgcheck();
							if(content==0){
								var y=art.dialog({title : '任务申诉',content : '投诉必须上传图片！',okValue : "确定",ok : function(){y.close();win.show();},lock : true});
								return false;
							}else{
								$.post('taobao.php?mod=appeal',{"id":id,"info":info,"content":content},function(data){
									if(data.state){
										art.dialog({title: '提示',content: data.info,fixed: true,lock: true,okValue:'确定',ok:function(){location.reload(); return true; }});
									}else{
										var aa=art.dialog({title: '提示',content: data.info,fixed: true,lock: true,okValue:'确定',ok:function(){aa.close();win.show();}});
									}
								},'json');
								win.show();
							}
			...
			art.dialog({title : '温馨提示',
			  ...
			  ok : function(){
					...
					$.post('?mod=pingjia&id='+id,{"grade":grade,"remark":remark,"isHide":isHide},function(data){
						// alert(data);
						if(data=='ok'){
							art.dialog({title : '温馨提示',content : "恭喜，评论成功，谢谢您对好会刷平台所做的贡献",okValue : "确定",ok : function(){location.reload();return true;},lock : true});	
						}
						return true;
					});
			...
			padding:0,
				  ...
				  ok : function(){
						...
						if(task_id=='' || address=='' || add_uer=='' || add_phone=='' || add_time=='' || add_pay=='' || money==''){
							art.dialog({title : '温馨提示',content : "底单信息填写不完整！",okValue : "确定",ok : function(){return true;},lock : true});	
						}else{
		...
		art.dialog({
				...
				ok:function(){
					...
					if(wl==''||addname==''||send_add==''||to_add==''||sendname==''||phone==''){
						art.dialog({title : '温馨提示',content : "购买底单信息填写不完整！",okValue : "确定",ok : function(){return true;},lock : true});	
					}else{
			...
			var win=art.dialog({
				...
				ok:function(){	
					...
					$(".url-upfile").each(function(){
						n=$(this).attr('alt');
						if($(this).val()==''){
							art.dialog({title : '温馨提示',content :'请上传货比第'+n+'家商品的截图！',okValue : "确定",ok : function(){return true;},lock : true});
							return false;
						}else{
							num+=1;
						}
					...
					if(num==n){
						art.dialog({id:'123',title : '温馨提示',content :'上传成功，请根据提示验证来路！',okValue : "确定",ok : function(){win.close();yz(id);return true;},lock : true});
						return false;
					}
					return false;
				},
				...
				ok:function(){	
					...
					if(ue){
						...
						if(!expcontarr || expcontarr.length != lxnum){
							art.dialog({id:'temp-info',title : '温馨提示',content :'此任务需要额外上传该店内其他'+lxnum+'件商品的截图，才能验证商品！('+lxnum+'张截图)',okValue : "确定",ok : function(){return true;},lock : true});
							img=0;
							return false;
						}else{
					...
					if (goodsurl==''){
						art.dialog({
							...
							content :'商品URL不能为空',
							okValue : "确定",
							ok : function(){return true;},lock : true
						});
						return false;
					}
					...
					if(suess=='商品地址验证成功！'&&img==1){
						...
						var url=$('input[name=goodsurl]').val();
						var ids=$('#verifyGoods').attr('ids');
						$.post('?mod=istrueurl',{data:url,id:ids},function(backdata){
							if(backdata==1){
								$.post('?mod=yzsuccess',{
									taskid:$('#verifyGoods').attr('ids'),
									url:$('input[name=goodsurl]').val()
								},function(backdata){
									if(backdata==1){
										$(".d-outer d-state-focus d-state-visible").parent().remove();
										var x=confirm("验证成功");
										if(x==true||x==false){
											location.reload();
										}
