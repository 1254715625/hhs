$(function(){
	$("#add_bank_btn").click(function(){
		...
		if(data==0){
			art.dialog({id:'payment', title:'温馨提示', lock:true,
			...
			art.dialog({id:'payment', title:'添加提现卡号', lock:true,
				...
				/*if(!/^[\u4e00-\u9fa5].+$/gi.test(bank_name)){
					art.dialog({title: '提示',content: '真实姓名含有非汉字字符',fixed: true,lock: true});return false;
				}*/
				if(bank_name==13){
					if(bank_num == ""){
	...
	$('.js-bank-list').click(function(){
			...
			$(".xcont").click(function(){
				...
				art.dialog({
					...
					ok: function () {
						...
						if(arr[1]==13){
							if(cont == ''){ 
								art.dialog({id:'ts', title: '提示',content: '请填写正确的支付宝账户~',fixed: true,cancelValue: '确定',cancel: function () { return true;}});
								return false;
							}
						}else{
							if(cont == '' || !(/\d{16,20}/.test(cont))){ 
								art.dialog({id:'ts', title: '提示',content: '请填写正确的银行帐号~',fixed: true,cancelValue: '确定',cancel: function () { return true;}});
								return false;
							}
						}
						...
						$.post('user.php?act=payment&opt=xcont',{"bank":arr[0],"cont":cont,'type':arr[1]},function(data){
							...
							art.dialog({title: '提示',content: data.info,fixed: true,lock: true,cancelValue: '确定',cancel: function () { location.reload(); }});
						},'json');
					},
					cancel: function () { return true;}
				});
		...
		$('#amount').keyup(function(){
			...
			if(money){
	           	if(money < 100){
					var nc = '本次提现金额为：<b style="color:red;">'+money+'</b>元，手续费<b style="color:red;">'+price+'</b>元，实际到账<b style="color:red;">'+(money-price)+'</b>元。<a href="javascript:;" style="margin-left: 10px;color: red;text-decoration: underline;" id="shimoe" onclick="shimoe('+count+');">与实际余额不符？</a><br><span style="color:#B4B4B4;">（100元以上提现免手续费）</span>';
				}else{
		...
		$('.tjsq_btn').click(function(){
			...
			if(m ==''){
				art.dialog({id:'mention', title: '提示',content: '提现金额不能为空~',fixed: true,lock: true,cancelValue: '确定',cancel: function () { return true;}});return false;
			}

			if(m > sy){
				art.dialog({id:'mention', title: '提示',content: '对不起，您填写的提现金额大于可提现余额~',fixed: true,lock: true,cancelValue: '确定',cancel: function () { return true;}});return false;
			}
			if(cont == undefined){
				art.dialog({
					id:'mention', 
			...
			if(m < 100){
				var content = '您好，你提现的金额为 <b style="color:red;">￥'+m+'</b>，手续费<b style="color:red;">￥'+price+'</b> 元<hr></hr>普通用户提现金额低于100元，需要支付提现手续费2元。<br>商保用户、VIP用户 免手续费。<br/><p style="margin:10px 0;color:#EB0F0F;">请输入安全码：<input style="border:1px solid #999;" type="password" id="safecode"></p>';
			}else{
			...
			art.dialog({
			    ...
			    ok: function () {
			    	...
			    	$.post('user.php?act=payment&opt=mention',{"mo":m, "sms":s,"bank":arr[0],"safecode":$("#safecode").val()},function(data){
			    		...
			    		art.dialog.get('ajaxStart').close();
			    		if(data.status == 1){
			    			art.dialog({title: '提示',content: data.info,fixed: true,lock: true,cancelValue: '确定',cancel: function () { location.href="user.php?act=payment_list";}});
			    		}else{
...
function changebank(){
	$('#addbank').change(function(){
	var val=$(this).val();
	if(val==13){
		$('#name').html('支付宝名称：');
		$('#banks').html('支付宝账户：');
		$('.card_name').attr('placeholder','请输入支付宝名称');
		$('.card_number').attr('placeholder','请输入支付宝账户');
	}else{
		$('#name').html('真实姓名：');
		$('#banks').html('您的银行账号：');
		$('.card_name').attr('placeholder','如银行户名：李四');
		$('.card_number').attr('placeholder','如银行账号：6222023100032381111');
	}
});
}