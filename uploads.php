...
if($payment){
	$this_attr=$db->getField("mcs_task_paipai a,mcs_member_bindacc b","attrs","a.id=$payment and a.get_user=b.id and b.uid=$uinfo[user_id] and a.process=0");
	if($this_attr){
...
$(function () {
	...
	var files = $(".files");
	var btn = $(".btn span");
	$(".demo").wrap("<form id='myupload' action='actions.php?payment=<?php echo $payment;?>' method='post' enctype='multipart/form-data'></form>");
